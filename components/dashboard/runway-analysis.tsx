...
// Common runway configurations for airports (using typical patterns)
const RUNWAY_CONFIGS: RunwayConfig[] = [
  { name: "01/19", heading: 10 }, // ~North-South
  { name: "03/21", heading: 30 }, // NE-SW
  { name: "06/24", heading: 60 }, // E-W
]
...
export function RunwayAnalysis({
  ...
  const analyzeRunway = (runway: RunwayConfig): RunwayAnalysis => {
    // Normalize wind direction to runway heading
    ...
    // Crosswind component (absolute value, always positive)
    const crosswind = Math.abs(windSpeed * Math.sin((headingDiff * Math.PI) / 180))

    // Runway is optimal if:
    // - Headwind is positive (wind coming from front)
    // - Crosswind is less than 10 km/h (typical limit)
